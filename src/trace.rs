...
pub fn trace(cpu: &mut CPU) -> String {
    ...
    let origin = cpu.program_counter;
    ...
    let tmp = match ops.len {
        ...
        2 => {
            let address: u8 = cpu.mem_read(origin + 1);
            ...
            match ops.mode {
                ...
                AddressingMode::NoneAddressing => {
                    ...
                    let address: usize =
                        (origin as usize + 2).wrapping_add((address as i8) as usize);
                    format!("${:04x}", address)
                }
        ...
        3 => {
            let address_lo = cpu.mem_read(origin + 1);
            let address_hi = cpu.mem_read(origin + 2);
            hex_dump.push(address_lo);
            hex_dump.push(address_hi);

            let address = cpu.mem_read_u16(origin + 1);
    ...
    .collect::<Vec<String>>()
    .join(" ");
    let asm_str = format!("{:04x}  {:8} {: >4} {}", origin, hex_str, ops.mnemonic, tmp)
        .trim()
        .to_string();
